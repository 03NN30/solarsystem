...
void drawPlanetViewer(Node &it) {
  ...
  if (isFollowing) {
    Camera::get().follow(objectToFollow->getWorldTransform(), defaultDirection,
                         objectToFollow->getSize() + 1.0f);
  }
...
void drawCameraViewer() {
  ...
  ImGui::Text("Position: %f, %f, %f", Camera::get().position[0],
              Camera::get().position[1], Camera::get().position[2]);
  ImGui::Text("Front:    %f, %f, %f", Camera::get().front[0],
              Camera::get().front[1], Camera::get().front[2]);
  ImGui::Text("WorldUp:  %f, %f, %f", Camera::get().worldUp[0],
              Camera::get().worldUp[1], Camera::get().worldUp[2]);
  ImGui::Text("Up:       %f, %f, %f", Camera::get().up[0], Camera::get().up[1],
              Camera::get().up[2]);
  ImGui::Text("Right:    %f, %f, %f", Camera::get().right[0],
              Camera::get().right[1], Camera::get().right[2]);
}

void drawDebugViewer() {}

void drawDeactivateFollowing() {
  ImGui::Begin("deactivate following", nullptr,
...
void drawHelp() {
  ...
  const char *explanation = "Just right click your screen to swap between the\n"
                            "overlay and the controller.";
  const char *credits = "This software was created by me using:            ";

  const char *disclaimer = "I do NOT own any texture files used in this app.  ";

  const char *feedback =
      "THE CODE IS SO BAD ... I AM ASHAMED OF MYSELF (3 years later)";
...
bool drawMenu() {
  if (menu) {
    ...
    ImGui::SameLine();
    if (!showSettings && !showCameraViewer && !showPlanetViewer &&
        !showDebugViewer && !showHelp) {
      if (ImGui::Button("+")) {
        showSettings = true;
    ...
    } else if (showSettings && showCameraViewer && showPlanetViewer &&
               showDebugViewer && showHelp) {
      if (ImGui::Button("-")) {
        showSettings = false;
    ...
    if (showPlanetViewer) {
      ...
      ImGui::Begin("Planet Viewer##2", nullptr,
                   ImGuiWindowFlags_AlwaysAutoResize);
      drawPlanetViewer(*sg);
      ImGui::Separator();
    ...
    if (showCameraViewer) {
      ...
      ImGui::Begin("Camera Viewer##2", nullptr,
                   ImGuiWindowFlags_AlwaysAutoResize);
      drawCameraViewer();
      ImGui::Separator();
    ...
    if (showDebugViewer) {
      ...
      ImGui::Begin("Debug Viewer##2", nullptr,
                   ImGuiWindowFlags_AlwaysAutoResize);
      drawDebugViewer();
      ImGui::Separator();
